  ...
  return _jsx(AppLayout, {
    children: _jsxs("div", {
      ...
      children: [
        ...
        _jsxs("section", { className: "section-card product-selector", children: [_jsxs("div", { children: [_jsx("p", { className: "card-label", children: "Carga transportada" }), _jsx("h2", { className: "section-title", children: "Última carga" }), _jsx("p", { className: "helper", children: "Selecione um produto cadastrado. O procedimento é informativo e não bloqueia a inspeção." })] }), _jsx("input", { className: "input", value: productQuery, placeholder: "Digite nome, ONU, sinônimo ou nome químico...", onChange: (e) => { setProductQuery(e.target.value); if (e.target.value !== selectedProduct?.name) setSelectedProduct(null); } }), productOptions.length ? _jsx("div", { className: "product-suggestions", children: productOptions.map((product) => _jsxs("button", { type: "button", onClick: () => { setSelectedProduct(product); setProductQuery(product.name); setProductOptions([]); }, children: [_jsxs("strong", { children: [product.name, product.unNumber ? ` · ONU ${product.unNumber}` : ""] }), _jsxs("span", { children: [product.family?.name || "Sem família", " · ", product.manufacturer?.name || "Fabricante não informado"] })] }, product.id)) }) : null, selectedProduct ? _jsxs("div", { className: "selected-product", children: [_jsxs("div", { children: [_jsx("strong", { children: selectedProduct.name }), _jsx("span", { children: selectedProduct.family?.name || "Família não informada" })] }), _jsx("span", { className: `product-risk product-risk--${selectedProduct.riskLevel.toLowerCase()}`, children: `Risco ${selectedProduct.riskLevel === "HIGH" ? "alto" : selectedProduct.riskLevel === "MEDIUM" ? "médio" : "baixo"}` }), _jsx("span", { className:`procedure-badge procedure-badge--${(selectedProduct.washingProcedure||"NOT_DEFINED").toLowerCase()}`, children: ({WASH_ONLY:"Lavagem sem vapor",STEAM_ONLY:"Somente vapor",WASH_AND_STEAM:"Lavagem + vapor",NO_WASH_REQUIRED:"Não necessita lavagem",NOT_DEFINED:"Não definido"})[selectedProduct.washingProcedure||"NOT_DEFINED"] }), selectedProduct.washingProcedureNotes ? _jsx("p",{className:"selected-product__notes",children:`Observação: ${selectedProduct.washingProcedureNotes}`}):null] }) : null] }),
        _jsx(InspectionForm, {
          values,
