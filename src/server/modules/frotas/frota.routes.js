...
frotaRoutes.get("/numero/:numeroFrota/historico", async (req, res, next) => {
    try {
        ...
        return res.json({
            ...
            resumoRecorrencia: {
                itensRecorrentes,
                mensagemResumo: itensRecorrentes.length > 0
...
frotaRoutes.patch("/:id", async (req, res, next) => {
    try {
        ...
        const data = {};
        if (typeof payload.numeroFrota === "string")
            data.numeroFrota = payload.numeroFrota.trim();
        if (typeof payload.placa === "string")
            data.placa = payload.placa.trim();
        if (typeof payload.tipoEquipamento === "string")
            data.tipoEquipamento = payload.tipoEquipamento.trim();
        if (typeof payload.material === "string")
            data.material = payload.material.trim();
        if (typeof payload.capacidade === "string")
            data.capacidade = payload.capacidade.trim();
        if (typeof payload.observacoesFixas === "string")
            data.observacoesFixas = payload.observacoesFixas.trim();
        if (payload.observacoesFixas === null)
            data.observacoesFixas = null;
